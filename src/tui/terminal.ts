...
export class Terminal {
   ...
   * 尺寸变化后的重锚。
   *
   * 重排会让「活动区占几行、光标在第几行」全部失效，但不能像早先那样「只重置记账」——
   * 那样下一次重绘会变成在旧副本下面**追加**一份；拖动窗口每步抛一次 resize，屏幕上
   * 就会叠出几十份残影。
   *
   * 这里改成：从光标当前所在的这一行往下擦干净再重置记账。活动区的输入行通常就是光标
   * 所在行，因此重绘会正好覆盖回原位，不残留也不误删已提交内容。
   * 残留上限：活动区里位于输入行之上的那几行（浮层标题/明细，最多 4 行）。
   */
  resetLive(): void {
    if (this.liveRows > 0) process.stdout.write(`\r${ansi.eraseToEnd}`);
    this.liveRows = 0;
    this.cursorRow = 0;
  }
}
