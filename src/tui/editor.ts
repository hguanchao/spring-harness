...
export class Editor implements Component, Focusable {
	...
	handleMouse(event: TuiMouseEvent): TuiMouseEventResult | undefined {
		...
		const visualLines = this.buildVisualLineMap(this.lastWidth);
		const visualLineIndex = this.scrollOffset + boxRow - 1;
	...
	handleInput(data: string): void {
		...
		// 内联菜单以最高优先级接管按键:列表键位(↑↓/Enter/Esc)交给菜单,其余键一律吞掉。
		// Ctrl+C 退出走 UI 层输入监听器,不经过这里。
		if (this.inlineMenu) {
			if (this.inlineMenu.filterable && this.applyMenuFilter(data)) return;
			this.inlineMenu.list.handleInput(data);
			return;
		}

		const paste = consumeBracketedPaste(data, this.paste);
		this.paste = paste.state;
		if (paste.consume) {
			if (paste.paste !== undefined && paste.paste.length > 0) this.handlePaste(paste.paste);
		...
		if (this.autocompleteState && this.autocompleteList) {
			...
			if (kb.matches(data, "tui.input.tab")) {
				...
				if (selected && this.autocompleteProvider) {
					this.applyCompletion(this.autocompleteProvider, selected, this.autocompletePrefix);
					this.cancelAutocomplete();
					this.notifyChange();
				}
				return;
	...
	private handleBackspace(): void {
		...
		if (this.autocompleteState) {
			...
			// If autocomplete was cancelled, re-trigger in the current context.
			this.tryTriggerAutocompleteForCurrentContext();
		}
	}
	...
	private deleteToEndOfLine(): void {
		...
		if (this.state.cursorCol < currentLine.length) {
			...
			this.mergeWithNextLine(currentLine);
		}

		this.notifyChange();
	}

	/** 行尾删除换行：把当前行与下一行合并成一行。 */
	private mergeWithNextLine(currentLine: string): void {
	...
	private createAutocompleteList(
		...
		// 滚动计数由菜单盒的底边框承载，不在正文里渲染。
		list.renderScrollInfoLine = false;
		list.onSelect = (selected) => {
			const provider = this.autocompleteProvider;
			if (!provider) return;
			this.applyCompletion(provider, selected, this.autocompletePrefix);
			this.cancelAutocomplete();
			this.notifyChange();
		};
		return list;
	...
	/**
	 * 应用一条补全：压栈快照 → 交给 provider 算出新文本与光标 → 写回编辑器状态。
	 * Tab、Enter、列表点选、唯一候选自动补全四处共用。
	 */
	private applyCompletion(provider: AutocompleteProvider, item: AutocompleteItem, prefix: string): void {
		this.pushUndoSnapshot();
		this.lastAction = null;
	...
	public isShowingAutocomplete(): boolean {
		// 内联菜单也算「补全在显示」:宿主的 Escape 中断逻辑要让位给菜单关闭。
		return this.autocompleteState !== null || this.inlineMenu !== undefined;
	}
	...
	/**
	 * 过滤模式下吃掉可打印字符、退格和括号粘贴。
	 * 方向键、回车、Esc 返回 false，交给列表自己处理。
	 */
	private applyMenuFilter(data: string): boolean {
		const menu = this.inlineMenu;
		if (!menu) return false;
		const paste = consumeBracketedPaste(data, this.paste);
		this.paste = paste.state;
		if (paste.consume) {
			if (paste.paste !== undefined && paste.paste.length > 0) this.pushMenuQuery(paste.paste);
			if (paste.remaining) this.handleInput(paste.remaining);
			return true;
		}
		...
		if (data.length === 1 && data >= ' ' && data <= '~') {
			this.pushMenuQuery(data);
			return true;
		}
		return true;
	}

	private pushMenuQuery(text: string): void {
		const menu = this.inlineMenu;
		if (!menu) return;
		...
		if (text.length > 1 && menu.list.itemCount === 1) {
			...
			if (item) this.closeInlineMenu(item);
		}
	}

	private closeInlineMenu(item: SelectItem | undefined): void {
		const menu = this.inlineMenu;
		if (!menu) return;
	...
	private tryTriggerAutocompleteForCurrentContext(): void {
		const currentLine = this.state.lines[this.state.cursorLine] || "";
		const textBeforeCursor = currentLine.slice(0, this.state.cursorCol);
		if (this.isInSlashCommandContext(textBeforeCursor) || this.autocompleteTriggerPattern.test(textBeforeCursor)) {
			this.tryTriggerAutocomplete();
		}
	}
