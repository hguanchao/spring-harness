...
export interface SessionCommandHost {
  ...
  startSession(next: JsonlSession): void;
...
export async function commandNewSession(host: SessionCommandHost): Promise<void> {
  ...
  if (!confirmed) return;
  const next = createSession(host.deps.sessionDir, host.deps.workspaceRoot);
...
export async function commandResume(host: SessionCommandHost, id: string): Promise<void> {
  ...
  if (id !== '') {
    const sessions = await listSessions(deps.sessionDir, { includeSubagents: true });
  ...
  const selected = await host.editor.showInlineMenu({ title: 'Sessions', items, maxVisible: 12, primaryColumnWidth: primaryColumnWidthFor(items) });
...
export async function commandExport(host: SessionCommandHost, argument: string): Promise<void> {
  ...
  const body = format === 'json'
    ? exportJson(host.session)
    : format === 'html'
      ? exportHtml(host.session)
      : exportMarkdown(host.session);
