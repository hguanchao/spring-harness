...
/** 把一段渲染结果包进 OSC 133 区块：首行加起始标记，末行加结束标记。 */
...
/** 超出列宽时按列裁切；已在宽度内则原样返回。 */
export function clipLineToWidth(line: string, width: number): string {
	return visibleWidth(line) <= width ? line : sliceByColumn(line, 0, width, true);
}
