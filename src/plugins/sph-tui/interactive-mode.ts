...
class InteractiveMode implements ApprovalUi, SteerBarHost, TranscriptHost, ReplayHost, SessionCommandHost, SettingsCommandHost {
  ...
  public startSession(next: SessionPort): void {
    try {
  ...
  public switchToSession(id: string): void {
    ...
    const sessions = this.deps.pluginServices.get<SessionService>(SESSION_SERVICE) ?? sessionService;
    this.session = (this.deps.sessions ?? sessions.factory).open(this.deps.sessionDir, id);
    sessions.activate(this.deps.sessionDir, id, this.deps.workspaceRoot);
    this.clearChat();
    this.resetRecapState();
    this.restoreSession();
    this.refreshCounters();
   ...
   */
  private sessionContext(messages: ReturnType<SessionPort['readMessages']>): RecapContext {
    return {
      messages,
