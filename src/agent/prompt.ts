...
export function buildSystemPrompt(input: {
  ...
  return [
    ...
    'Tools: read_file, write, search_replace, grep, list_dir, shell, skill, todo, ask_user, web_fetch, jobs, subagent, send_subagent_message, mcp.',
    'Use search_replace for edits. write is for new files or full rewrites.',
    'shell is PowerShell (pwsh) on Windows, sh on Unix. cwd is the workspace root. Each call is one-shot; there is no long-lived shell.',
    'Use skill(name) to load a SKILL.md. Catalog is name + description only.',
    'Use todo to keep a short in-session checklist. Use subagent to fan out work (explore is read-only). Give every subagent call a description of 3-5 words — it is the only label the user sees on that row. background: true returns immediately — you will be notified automatically when it completes; jobs lists/gets status snapshots only and does not start shells.',
