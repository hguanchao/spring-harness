...
export const subagentTool: ToolSpec = {
  name: 'subagent',
  description:
    `Run a child agent with its own session and block until it finishes — its final report comes back as this tool's result. Use this whenever your answer depends on the child's findings, and send several subagent calls in the same reply to run them in parallel (up to ${SUBAGENT_CONCURRENCY}). Nesting is flat by default (depth 1): a subagent cannot spawn its own subagents, and a call beyond the configured depth budget fails with an explicit depth error. type: explore (read-only tools) or general (can edit). background: true is ONLY for fire-and-forget chores whose result your reply does not depend on — it returns a job id immediately and you will be notified automatically when it completes; use jobs(action:get) for a status snapshot only. Long-running work goes here, not through a background shell. isolation: worktree runs the child in an isolated git worktree (its result reports the path). resume_from: pass a completed subagent's session id to continue its conversation.`,
  schema: {
    type: 'object',
    properties: {
